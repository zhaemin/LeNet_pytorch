...
<<<<<<< Updated upstream
    ...
    parser.add_argument('-e', '--epochs', metavar = 'int', type = int, help = 'epochs (int)', default = 2 )
    parser.add_argument('-lr', '--learningrate', metavar = 'float', type = float, help = 'lr (float)', default = 0.001 )
    parser.add_argument('-bs', '--batchsize', metavar = 'int', type = int, help = 'batchsize (int)', default = 4)
    ...
    parser.add_argument('-tr', '--train', help = 'train (bool)', action='store_true')
    parser.add_argument('-tc', '--test', help = 'test (bool)', action='store_true')
    parser.add_argument('-p', '--patience', type= int, help = 'patience (int)', default=10)
...
def main():
    ...
    outputs_log = open(f'/home/work/study/haemin/ResNet/outputs/{args.model}_{args.epochs}ep_{args.learningrate}lr_{args.optimizer}_{cur_time}.txt','w')
    writer = SummaryWriter(f'/home/work/study/haemin/ResNet/logs/{args.model}_{args.epochs}ep_{args.learningrate}lr_{args.optimizer}_{cur_time}')
    
    if args.train:
        train.train_main(args.epochs, args.batchsize, args.dataset, args.model, args.learningrate, args.optimizer, device, writer, args.patience, outputs_log)
    if args.test:
        test.test_main(args.batchsize, args.dataset, args.model, device, writer, outputs_log)
...
def main():
    ...
    net = model.set_net(args.model, True, num_classes)
    net.to(device)
    ...
    if args.test:
        print(f"Test start ...")
        net = model.set_net(args.model, False, num_classes)
